...
| **`e2e/`** | Интеграционные e2e-тесты (Playwright): сценарии в [`e2e/SCENARIOS.md`](e2e/SCENARIOS.md), запуск через задачи `e2e:*` в [`.mise.toml`](.mise.toml). |
...
Сейчас в `[tasks]` есть, например: `spec:*`, `front:*`, `back:*`, `prism:mock`, `e2e:install`, `e2e:test`, `e2e:ci` и др. При расхождении с README или с этим файлом **верьте `.mise.toml`**.
